...
def plot_acc_curves(accRecord, ax, title='default', saveFolder="./"):
    ...
    ax.set_title(format(title))
    ax.legend()
def plot_combined_acc(folder = "./accLoss", title='combine', saveFolder="./plot"):
    
    fig, axs = plt.subplots(3, 1, figsize=(10, 8), sharex=True, constrained_layout=True)
    for kth in range(3):
        trainNasTrainAccFile = os.path.join(folder, "retrainTrainAcc_{}.npy".format(str(kth)) )
        trainNasnValAccFile = os.path.join( folder,"retrainValAcc_{}.npy".format(str(kth)) )
...
def plot_alphas(accRecord, ax, title='default', saveFolder="./"):
    ax.plot(accRecord['train'], c='tab:red', label='train')
    ax.plot(accRecord['val'], c='tab:cyan', label='val')
    try:
        ax.plot(accRecord['test'], c='tab:brown', label='test')
...
if __name__=="__main__":
    ...
    for kth in range(1):
        trainNasTrainAccFile = os.path.join(folder, "retrainTrainAcc_{}{}.npy".format(str(kth), net) )
        trainNasnValAccFile = os.path.join( folder,"retrainValAcc_{}{}.npy".format(str(kth), net) )
        testAccFile = os.path.join(folder, "testAcc_{}.npy".format(str(kth)) )
